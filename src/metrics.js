 ...
 * with the human attendant's name, e.g. "<strong>Jonas</strong>: bom dia".
 */
 ...
 * next HUMAN agent reply (bot auto-replies are ignored).
 * `pendingUserTs` (ts of the oldest still-unanswered customer message) is
 * returned so callers can recompute "waiting for how long" at any later time.
 */
...
/** Time-decay for cached temperatures: a lead idle for 72h+ without strong
 * intent cools down to "frio" even if the cached content-score said "morno". */
export function decayTemperatura(temperatura, interesseScore, horasDesde) {
  if (temperatura !== 'frio' && (horasDesde ?? 0) > 72 && (interesseScore ?? 0) < 4) return 'frio';
  return temperatura;
}
