...
const SYSTEM = 'Você avalia a qualidade do atendimento de VENDEDORES em conversas de WhatsApp de uma loja de móveis e colchões. '
...
export function createScorer() {
  ...
  return {
     ...
     *  @param messages normalized transcript [{sender, text}] */
    ...
    flush() {
      if (!dirty) return;
      ensureDirs();
      fs.writeFileSync(AI_SCORES_FILE, JSON.stringify(pruneOldEntries(map, config.doneRetentionDays), null, 2));
      dirty = false;
    },
  };
}
